import tkinter, turtle, xml
from classesExample import GoToCommand, CircleCommand, BeginFillCommand, EndFillCommand, PenDownCommand, PenUpCommand, PyList
...
class DrawingApplication(tkinter.Frame):
    ...
    def buildWindow(self):
        ...
        def parse(filename):
            ...
            graphicsCommandElement = xmldoc.getElementsbyTagName("GraphicsCommand")[0]
            ...
            for commandElement in graphicsCommands:
                ...
                if command == "GoTo":
                    ...
                    width = float(attr["width"].value)
                    color = attr["color"].value.strip()
                    cmd = GoToCommand(radius, width, color)

                elif command == "BeginFill":
                    color = attr["color"].value.strip()
                    cmd = BeginFillCommand(color)
                
                elif command == "EndFill": cmd = EndFillCommand()
                
                elif command == "PenUp": cmd = PenUpCommand()

                elif command == "PenDown": cmd = PenDownCommand()

                else: raise RuntimeError("Unknown Command: " + command)
        ...
        def getPenColor():
            ...
            if color != None: penColor.set(str(color)[-9:-2])
        ...
        def getFillColor():
            color = tkinter.colorchooser.askcolor()
            if color != None: fillColor.set(str(color)[-9:-2])
