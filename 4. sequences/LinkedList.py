# class Node:
#     def __init__(self, item, next=None):
#         self.item = item
#         self.next = next
    
#     def getItem(self):
#         return self.item

#     def getNext(self):
#         return self.next
    
#     def setItem(self, item):
#         self.item = item
    
#     def setNext(self, next):
#         self.next = next

class LinkedList:
    class __Node:
        def __init__(self, item, next=None):
            self.item = item
            self.next = next
        
        def getItem(self):
            return self.item

        def getNext(self):
            return self.next
        
        def setItem(self, item):
            self.item = item
        
        def setNext(self, next):
            self.next = next
    
    def __init__(self, contents=[]):
        self.first = LinkedList.__Node(None, None)
        self.last = self.first
        self.numItems = 0

        for e in contents:
            self.append(e)
    
    def __getitem__(self, index):
        if index >= 0 and index < self.numItems:
            cursor = self.first.getNext()
            for _ in range(index):
                cursor = cursor.getNext()

            return cursor.getItem()

    def __setitem__(self, index, val):
        if index >= 0 and index < self.numItems:
            cursor = self.first.getNext()
            for _ in range(index):
                cursor = cursor.getNext()

            cursor.setItem(val)
            return

        raise IndexError("LinkedList assignment index out of range")

    def __add__(self, other):
        if type(self) != type(other):
            raise TypeError("Concatenate undefined for " + \
                str(type(self)) + " + " + str(type(other)))
        
        result = LinkedList()

        cursor = self.first.getNext()

        while cursor != None:
            result.append(cursor.getItem())
            cursor = cursor.getNext()

        cursor = other.first.getNext()

        while cursor != None:
            result.append(cursor.getItem())
            cursor = cursor.getNext()

        return result    
    
    def append(self, item):
        node = LinkedList.__Node(item)
        self.last.setNext(node)
        self.last = node
        self.numItems += 1

    def insert(self, index, item):
        cursor = self.first

        if index < self.numItems:
            for _ in range(index):
                cursor = cursor.getNext()
            
            node = LinkedList.__Node(item, cursor.getNext())
            cursor.setNext(node)
            self.numItems += 1
        
        else: self.append(item)

    def __delitem__(self, item):
        cursor = self.first
        # print(cursor.getItem())

        while cursor.getNext().getItem() != item:
            cursor = cursor.getNext()
            # print(cursor.getItem(), end=" --> ")
            # print(cursor.getNext().getItem())

        cursor.setNext(cursor.getNext().getNext())
        # print(cursor.getNext().getItem())

    def __len__(self):
        return self.numItems

    def __iter__(self):
        cursor = self.first

        for _ in range(self.numItems):
            cursor = cursor.getNext()
            yield cursor.getItem()

    def __eq__(self, other):
        if type(self) != type(other): return False

        if self.numItems != other.numItems: return False

        cur = self.first
        oth = other.first

        for _ in range(self.numItems):
            cur = cur.getNext()
            oth = oth.getNext()
            
            if cur.getItem() != oth.getItem():
                return False
        
        return True

    def __contains__(self, item):
        cursor = self.first

        for _ in range(self.numItems):
            cursor = cursor.getNext()

            if cursor.getItem() == item:
                return True

        return False

x = LinkedList([1, 2, 3, 4])
y = LinkedList([1, 2, 3, 4])
z = LinkedList([1, 2, 3])
a = LinkedList([1, 2, 5, 4])
b = [1, 2, 3, 4]

print(x)
x.append(4)